...
- `toggles.plan_interview` (on by default) allows `wand plan
  ...
  quietly resolved.
- `toggles.plan_critic` (on by default) inserts a cold critic between the
  ...
  cold reviser. It spends a whole extra model call per plan run — the
  cheapest place in the lifecycle to spend one, since a plan is a decision
  paid for over the whole life of the work it describes. What the reviser
  resolves is recorded in the options comment as what was challenged and
  what changed; what it cannot resolve is promoted to an open question on
  that same comment, for a human to judge at Plan Review.
