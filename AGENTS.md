...
internal/schema/     shape-only JSON Schemas (keys, types, additionalProperties:
                     false — never a value constraint) generated by reflection
                     from a handoff's own Go struct, so the schema a
                     SchemaAdapter enforces can never drift from what the
                     Go validators parse
...
The same file holds `SchemaShape`, the live proof that a `SchemaAdapter`
actually gets `additionalProperties: false` enforced by the harness rather
than merely wired to a flag that turned out to be ignored — the two
harnesses are known to enforce different subsets of JSON Schema under
pressure, so this is checked live rather than assumed to hold across an
adapter upgrade.
